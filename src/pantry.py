...
class Pantry:
	...
	def load_ingredients(self, filepath: str):
		...
		self.__ingredients = {key: Ingredient(ing[0], ing[1], ing[2]) for key, ing in ingredients_as_lists.items()}

	def save_ingredients(self, filepath: str):
		ingredients_as_lists = {key: [ing.name, ing.quantity, ing.unit] for key, ing in self.__ingredients.items()}
	...
	def add_ingredient(self, name: str, qty: float = None, unit: str = None):
		...
		if name_normalized not in self.__ingredients:
			self.__ingredients[name_normalized] = Ingredient(name, qty, unit)
			return
	...
	def remove_ingredient(self, name: str):
		name_normalized = name.lower().replace(' ', '')
		if name_normalized in self.__ingredients:
			del self.__ingredients[name_normalized]
			return
	...
	def get_ingredient(self, name: str) -> list[Ingredient] | None:
		name_normalized = name.lower().replace(' ', '')
		if name_normalized in self.__ingredients:
			return self.__ingredients[name_normalized]
		return None
	...
	def edit_ingredient(self, name: str, field: Literal['name', 'qty', 'unit'], value: str | float):
		...
		if field == 'name':
			...
			if value_normalized in self.__ingredients:
				raise KeyError('Ingredient with this name already exists')
			self.__ingredients[value_normalized] = self.__ingredients[name_normalized]
			del self.__ingredients[name_normalized]
			self.__ingredients[value_normalized].name = value

		if field == 'qty':
			if not isinstance(value, float | int):
				raise TypeError('Value must be a float when editing quantity')
			self.__ingredients[name_normalized].quantity = float(value)

		if field == 'unit':
			if not isinstance(value, str):
				raise TypeError('Value must be a string when editing unit')
			if self.__ingredients[name_normalized].quantity == None:
				raise ValueError('Unit cannot be added when Quantity has value None')
			self.__ingredients[name_normalized].unit = value